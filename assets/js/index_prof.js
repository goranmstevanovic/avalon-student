...
// chart 2
$.getJSON('admin/api/profesor_casovi_mesec.php', function (response) {

    // Ako hoćeš baš siguran redosled i kad backend zeznu ORDER BY:
    response.sort((a, b) => (a.godina - b.godina) || (a.mesec - b.mesec));

    const labels = [];
    const zakazani = [];
    const odrzani = [];
    const procenat = []; // opcionalno

    const meseci = ['jan','feb','mar','apr','maj','jun','jul','avg','sep','okt','nov','dec'];

    response.forEach(row => {
        const m = parseInt(row.mesec, 10);
        const g = parseInt(row.godina, 10);

        const mesecNaziv = meseci[(m >= 1 && m <= 12) ? (m - 1) : 0];
        labels.push(`${mesecNaziv} ${g}`);

        const z = parseInt(row.broj_zakazanih, 10) || 0;
        const o = parseInt(row.broj_odrzanih, 10) || 0;

        zakazani.push(z);
        odrzani.push(o);

        // opcionalno: procenat realizacije
        procenat.push(z > 0 ? Math.round((o / z) * 100) : 0);
    });
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                // ✅ opcionalno: procenat realizacije kao linija
                {
                    label: 'Realizacija (%)',
                    type: 'line',
                    data: procenat,
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    beginAtZero: true,
                    title: { display: true, text: 'Broj termina' }
                },
                // ✅ desna osa za procenat (0–100)
                y1: {
                    position: 'right',
                    beginAtZero: true,
                    min: 0,
                    ...
                    title: { display: true, text: 'Realizacija (%)' }
                }
            }
        }
    });
